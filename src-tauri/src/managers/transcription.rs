    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        let result = {
            ...
            if let LoadedEngine::TranscribeCpp(session) = &engine {
                let model = session.model();
                let caps = model.capabilities();
                // Informational (logged below). The whisper run extension is gated on
                // `model_is_whisper`, not on this, since non-whisper archs can advertise
                // the feature yet reject the whisper-kind extension.
